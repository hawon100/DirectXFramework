...
bool SkinnedMeshDemo::Init()
{
	...
	_characterModel = make_shared<SkinnedModel>(_device, _texMgr, "../Resources/Models/soldier.m3d", L"../Resources/Textures/");
...
void SkinnedMeshDemo::DrawScene()
{
	...
	Effects::NormalMapSkinnedFX->SetDirLights(_dirLights);
	Effects::NormalMapSkinnedFX->SetEyePosW(_camera.GetPosition());
	Effects::NormalMapSkinnedFX->SetCubeMap(_sky->CubeMapSRV().Get());
	Effects::NormalMapSkinnedFX->SetShadowMap(smap->DepthMapSRV().Get());
	Effects::NormalMapSkinnedFX->SetSsaoMap(_ssao->AmbientSRV().Get());

	// Figure out which technique to use for different geometry.
	ComPtr<ID3DX11EffectTechnique> activeTech = Effects::NormalMapSkinnedFX->Light3TexTech;
	...
	ComPtr<ID3DX11EffectTechnique> activeSkinnedTech = Effects::NormalMapSkinnedFX->Light3TexSkinnedTech;
	...
	for (uint32 p = 0; p < techDesc.Passes; ++p)
	{
		...
		Effects::NormalMapSkinnedFX->SetWorld(world);
		Effects::NormalMapSkinnedFX->SetWorldInvTranspose(worldInvTranspose);
		Effects::NormalMapSkinnedFX->SetWorldViewProj(worldViewProj);
		Effects::NormalMapSkinnedFX->SetWorldViewProjTex(worldViewProj * toTexSpace);
		Effects::NormalMapSkinnedFX->SetShadowTransform(world * shadowTransform);
		Effects::NormalMapSkinnedFX->SetTexTransform(XMMatrixScaling(8.0f, 10.0f, 1.0f));
		Effects::NormalMapSkinnedFX->SetMaterial(_gridMat);
		Effects::NormalMapSkinnedFX->SetDiffuseMap(_stoneTexSRV.Get());
		Effects::NormalMapSkinnedFX->SetNormalMap(_stoneNormalTexSRV.Get());
		...
		Effects::NormalMapSkinnedFX->SetWorldViewProjTex(worldViewProj * toTexSpace);
		Effects::NormalMapSkinnedFX->SetShadowTransform(world * shadowTransform);
		Effects::NormalMapSkinnedFX->SetTexTransform(XMMatrixScaling(2.0f, 1.0f, 1.0f));
		Effects::NormalMapSkinnedFX->SetMaterial(_boxMat);
		Effects::NormalMapSkinnedFX->SetDiffuseMap(_brickTexSRV.Get());
		Effects::NormalMapSkinnedFX->SetNormalMap(_brickNormalTexSRV.Get());
		...
		for (int32 i = 0; i < 10; ++i)
		{
			...
			Effects::NormalMapSkinnedFX->SetWorldViewProjTex(worldViewProj * toTexSpace);
			Effects::NormalMapSkinnedFX->SetShadowTransform(world * shadowTransform);
			Effects::NormalMapSkinnedFX->SetTexTransform(XMMatrixScaling(1.0f, 2.0f, 1.0f));
			Effects::NormalMapSkinnedFX->SetMaterial(_cylinderMat);
			Effects::NormalMapSkinnedFX->SetDiffuseMap(_brickTexSRV.Get());
			Effects::NormalMapSkinnedFX->SetNormalMap(_brickNormalTexSRV.Get());
	...
	for (uint32 p = 0; p < techDesc.Passes; ++p)
	{
		...
		Effects::NormalMapSkinnedFX->SetWorldViewProjTex(worldViewProj * toTexSpace);
		Effects::NormalMapSkinnedFX->SetShadowTransform(world * shadowTransform);
		Effects::NormalMapSkinnedFX->SetTexTransform(XMMatrixScaling(1.0f, 1.0f, 1.0f));
		...
		{
			Effects::NormalMapSkinnedFX->SetMaterial(_characterInstance1.Model->Mat[subset]);
			Effects::NormalMapSkinnedFX->SetDiffuseMap(_characterInstance1.Model->DiffuseMapSRV[subset].Get());
			Effects::NormalMapSkinnedFX->SetNormalMap(_characterInstance1.Model->NormalMapSRV[subset].Get());
		...
		for (uint32 subset = 0; subset < _characterInstance1.Model->SubsetCount; ++subset)
		{
			Effects::NormalMapSkinnedFX->SetMaterial(_characterInstance2.Model->Mat[subset]);
			Effects::NormalMapSkinnedFX->SetDiffuseMap(_characterInstance2.Model->DiffuseMapSRV[subset].Get());
			Effects::NormalMapSkinnedFX->SetNormalMap(_characterInstance2.Model->NormalMapSRV[subset].Get());
